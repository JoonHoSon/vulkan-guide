...
void VulkanEngine::init() {
    ...
    if (const SDL_DisplayMode *displayMode =
        ...
        this->_windowExtent =
            VkExtent2D{.width = static_cast<uint32_t>(displayMode->w),
                       .height = static_cast<uint32_t>(displayMode->h)};
    }

    _window =
        SDL_CreateWindow("Vulkan Engine", static_cast<int>(_windowExtent.width),
                         static_cast<int>(_windowExtent.height), flags);

    SDL_GetWindowPosition(_window, &_lastWindowPosition.x,
    ...
    vkGetPhysicalDeviceSurfaceCapabilitiesKHR(_chosenGPU, _surface,
                                              &capabilities);
...
void VulkanEngine::draw() {
    ...
    vkUtil::transitionImage(command, _drawImage.image,
                            VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_GENERAL);

    drawBackground(command);

    vkUtil::transitionImage(command, _drawImage.image, VK_IMAGE_LAYOUT_GENERAL,
                            VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL);
    vkUtil::transitionImage(command, _swapChainImages[swapChainImageIndex],
                            VK_IMAGE_LAYOUT_UNDEFINED,
                            VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);

    vkUtil::copyImageToImage(command, _drawImage.image,
    ...
    const float flash = std::abs(std::sin(static_cast<float>(_frameNumber) /
    120.0f)); clearValue = {{0.0f, 0.0f, flash, 1.0f}};

    const VkImageSubresourceRange clearRange =
    vkInit::imageSubResourceRange(VK_IMAGE_ASPECT_COLOR_BIT);

    // Clear image
    vkCmdClearColorImage(command, _swapChainImages[swapChainImageIndex],
    ...
    now be executed) VK_CHECK(vkEndCommandBuffer(command));
    */

    VkCommandBufferSubmitInfo bufferSubmitInfo =
        vkInit::commandSubmitInfo(command);
    ...
    VkSubmitInfo2 submitInfo =
        vkInit::submitInfo(&bufferSubmitInfo, &signalInfo, &waitInfo);
...
void VulkanEngine::drawBackground(VkCommandBuffer buffer) const {
    // VkClearColorValue clearValue;
    ...
    // vkInit::imageSubResourceRange(VK_IMAGE_ASPECT_COLOR_BIT);
    //
    // vkCmdClearColorImage(buffer, _drawImage.image, VK_IMAGE_LAYOUT_GENERAL,
...
void VulkanEngine::run() {
    ...
    while (!quit) {
        ...
        if (const SDL_DisplayMode *displayMode =
                SDL_GetCurrentDisplayMode(SDL_GetPrimaryDisplay())) {
            changeSwapChain =
                _windowExtent.width != static_cast<uint32_t>(displayMode->w) ||
                _windowExtent.height != static_cast<uint32_t>(displayMode->h);
        }

        if (!changeSwapChain) {
            WindowPosition currentPosition{0, 0};

            SDL_GetWindowPosition(_window, &currentPosition.x,
                                  &currentPosition.y);

            changeSwapChain = _lastWindowPosition.x != currentPosition.x ||
                              _lastWindowPosition.y != currentPosition.y;
        }
...
void VulkanEngine::immediateSubmit(
    ...
    VkCommandBufferSubmitInfo cmdInfo = vkInit::commandSubmitInfo(cmd);
...
void VulkanEngine::initVulkan() {
    ...
    // Layer VK_LAYER_AMD_switchable_graphics uses API version 1.2 which is
    // older than the application specified API version of 1.4. May cause
    // issues.
    ...
    std::vector<vkb::PhysicalDevice> devices =
        selector.select_devices().value();
    ...
    for (const auto &dev : devices) {
        ...
        SPDLOG_DEBUG("Device type    : {}",
                     static_cast<int>(dev.properties.deviceType));
...
void VulkanEngine::initImgui() {
    ...
    //
    // 2025-06-11 ImGui_ImplVulkan_CreateFontsTexture(),
    // ImGui_ImplVulkan_DestroyFontsTexture() 제거됨
    //
...
void VulkanEngine::initSwapChain() {
    ...
    vmaCreateImage(_allocator, &imageInfo, &imageAllocationInfo,
...
void VulkanEngine::initCommands() {
    ...
    for (auto &frame : _frames) {
        ...
        VK_CHECK(vkAllocateCommandBuffers(_device, &allocate_info,
...
void VulkanEngine::createSwapChain(const uint32_t width,
...
void VulkanEngine::initDescriptors() {
    ...
    _drawImageDescriptors =
        globalAllocator.allocate(_device, _drawImageDescriptorLayout);
...
void VulkanEngine::initBackgroundPipelines() {
    ...
    VK_CHECK(vkCreatePipelineLayout(_device, &createInfo, nullptr,
